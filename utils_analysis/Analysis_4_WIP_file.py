...
path = os.getcwd() + '/result/P_2/Benchmarks/HP_3.json'
numbers = open(path)
    ...
    for u in urgencies:
        for i in range(len(data["orders"][0]["orders"])):
            if data["orders"][0]["orders"][i]["item_results"]["completion_time"] != 0:
                if data["orders"][0]["orders"][i]["priority"] == p and data["orders"][0]["orders"][i]["urgency"] == u:
                    ...
                    with open('WIP_' + str(p) + "_" + str(u) + '.csv', 'a+', newline='', encoding='utf-8') as file:
